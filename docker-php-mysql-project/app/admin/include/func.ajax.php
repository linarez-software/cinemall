...
switch($fn):
	...
	case 2: //Actualiza Entrada
		...
		case 5: //Inhablitar Butaca en Mapa de Sala
			$butaca = $_GET['but'];
			$id_sala = $_GET['ids'];
			$estado = $_GET['est'];
			$disponible = 0;
			$qry_butaca = "SELECT id_butaca, status FROM tbl_sala_butaca WHERE butaca = '".$butaca."' AND id_sala = ".$id_sala;
			$rst_butaca = mysql_query($qry_butaca);
			$disponible = mysql_num_rows($rst_butaca);
			if($campo = mysql_fetch_object($rst_butaca)):
				$qry = "DELETE FROM tbl_sala_butaca WHERE id_butaca = ".$campo->id_butaca;
				$result = mysql_query($qry);
			endif;
			
			if($estado > 0):
				$qry = "INSERT tbl_sala_butaca (id_sala, butaca, status) VALUES ($id_sala,'$butaca', $estado)";
				$result = mysql_query($qry);
			endif;
