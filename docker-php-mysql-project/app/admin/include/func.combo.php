...
$qry .= "ORDER BY o.usuario_registro ";
...
$tabla .= "<option value='6' ".(($seleccion == 6) ? ' selected' : '') . ">Oficina</option>";
$tabla .= "<option value='7' ".(($seleccion == 7) ? ' selected' : '') . ">Publica</option>";
$tabla .= "<option value='3' ".(($seleccion == 3) ? ' selected' : '') . ">Gerente</option>";
...
function combo_pelicula_funcion($activos = -1, $seleccion = 0){
$qry = "SELECT id_pelicula, nombre_espanol, duracion FROM tbl_pelicula ";
if ($activos > -1):
	$qry .= "WHERE activo = $activos ";
endif;
$qry .= "ORDER BY nombre_espanol ";
//echo $qry ;
$result = mysql_query($qry) or die("Error 007-func.combo - ".mysql_error());
$tabla = "";
$tabla .= "<select name='pelicula' id='pelicula' class='css-campo-metadata'>";
...
while($campo = mysql_fetch_object($result)):
	...
	$tabla .= "<option value='$campo->id_pelicula' $select >$campo->nombre_espanol [$campo->duracion min]</option>";
endwhile;
$tabla .= "</select>";
...
$tabla .= "<option value='".MATINEE."' ".(($seleccion == MATINEE) ? ' selected' : '') . ">Matinee</option>";
$tabla .= "<option value='".VESPERTINA."' ".(($seleccion == VESPERTINA) ? ' selected' : '') . ">Vespertina</option>";
$tabla .= "<option value='".INTERMEDIA."' ".(($seleccion == INTERMEDIA) ? ' selected' : '') . ">Intermedia</option>";
$tabla .= "<option value='".NOCHE."' ".(($seleccion == NOCHE) ? ' selected' : '') . ">Noche</option>";
$tabla .= "<option value='".MEDIANOCHE."' ".(($seleccion == MEDIANOCHE) ? ' selected' : '') . ">Media Noche</option>";
...
$tabla .= "<option value='".MATINEE."' ".(($seleccion == MATINEE) ? ' selected' : '') . ">Matinee</option>";
$tabla .= "<option value='".VIN."' ".(($seleccion == VIN) ? ' selected' : '') . ">V-I-N</option>";
...
function combo_tipo_sala($seleccion = 0, $nombre = 'tipo_sala'){
	$qry = "SELECT id_tipo_sala, tipo_sala FROM tbc_tipo_sala ";
	//echo $qry ;
	$result = mysql_query($qry) or die("Error 010-func.combo - ".mysql_error());
	$tabla = "";
	$tabla .= "<select name='tipo_sala' id='tipo_sala' class='css-campo-metadata'>";
	$tabla .= "<option value='0' selected >- Tipo Sala -</option>";
	while($campo = mysql_fetch_object($result)):
		if($seleccion == $campo->id_tipo_sala):
			$select = 'selected';
		else:
			$select = '';
		endif;
		$tabla .= "<option value='$campo->id_tipo_sala' $select >$campo->tipo_sala</option>";
	endwhile;
	$tabla .= "</select>";
	
	return $tabla;
}
function combo_formato_proyeccion($seleccion = 0, $nombre = 'formato_proyeccion'){
	$qry = "SELECT id_formato_proyeccion, formato_proyeccion FROM tbc_formato_proyeccion ";
	//echo $qry ;
	$result = mysql_query($qry) or die("Error 013-func.combo - ".mysql_error());
	$tabla = "";
	$tabla .= "<select name='formato_proyeccion' id='formato_proyeccion' class='css-campo-metadata'>";
	$tabla .= "<option value='0' selected >- Formato Proyeccion -</option>";
	while($campo = mysql_fetch_object($result)):
		if($seleccion == $campo->id_formato_proyeccion):
			$select = 'selected';
		else:
			$select = '';
		endif;
		$tabla .= "<option value='$campo->id_formato_proyeccion' $select >$campo->formato_proyeccion</option>";
	endwhile;
	$tabla .= "</select>";
	
	return $tabla;
}
function combo_procedencia($seleccion = 0, $nombre = 'procedencia'){
	$qry = "SELECT id_procedencia, procedencia FROM tbc_procedencia ";
	//echo $qry ;
	$result = mysql_query($qry) or die("Error 013-func.combo - ".mysql_error());
	$tabla = "";
	$tabla .= "<select name='procedencia' id='procedencia' class='css-campo-metadata'>";
	$tabla .= "<option value='0' selected >- Procedencia -</option>";
	while($campo = mysql_fetch_object($result)):
		if($seleccion == $campo->id_procedencia):
			$select = 'selected';
		else:
			$select = '';
		endif;
		$tabla .= "<option value='$campo->id_procedencia' $select >$campo->procedencia</option>";
	endwhile;
	$tabla .= "</select>";
