...
<?php include "head.php";?>
	...
	if(strlen($modulo) > 0):
		...
		$qry_id2 = "SELECT porcentaje,pe.id_procedencia FROM tbl_pelicula pe INNER JOIN tbl_programacion  pr  on  pe.id_pelicula = pr.id_pelicula  inner join tbl_distribuidor d ON d.id_distribuidor = pe.id_distribuidor WHERE pr.id_programacion id_programacion in ('$id_programacion') ";
		...
		$qry2222 = "SELECT id_configuracion, nombre_empresa, server_ip, logo_empresa, limite_funcion, rif, nit, direccion, telefono, municipal, fomprocine, alquiler,isv,tasa FROM tbl_configuracion ";
		//$qry .= "WHERE id_pelicula = $idc ";
		$resul2222t = mysql_query($qry2222);
		$fomprocine =  2.5 ;
		if($campo2222 = mysql_fetch_object($resul2222t)):
			$fomprocine =  $campo2222->fomprocine;
		endif;
		$pd = $fomprocine;
