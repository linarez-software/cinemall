...
$qry .= "WHERE DATE(tp.fecha_programacion) = '$fecha_actual' ";
	...
	if(mysql_num_rows($result) > 0):
		...
		while($campo=mysql_fetch_object($result)):
			...
			if($cine_actual != $campo->cine):
				...
				$url = '"funcion-extra.php?cine='.$campo->id_cine.'&fecha='.formato_fecha($fecha_actual, 2).'&sala='.$sala.'&pelicula='.$pelicula.'"';
				$table .= "<tr> \n";
				$table .= "<td colspan='6'><table cellpadding='0' cellspacing='0' border='0'><tr> \n";
				$table .= "<td align='left' height='21' class='pelicula-nombre'><b>CINE:</b> $cine_actual</td> \n";
				$table .= "<td align='left' height='21'>&nbsp;&nbsp;&nbsp;<a href='#' onclick='javascript:open_funcion($url)'><img src='images/add.png' border='0'></a></td> \n";
				$table .= "</tr></table></td>";
				$table .= "</tr> \n";
			endif;
			...
			$table .= "<td width='10%' align='right' class='info'>(".number_format($campo->boletos, 0, ',', '.').")&nbsp;<input align='right' type='text' size='5' id='ent$campo->id_programacion$campo->id_precio' value='".number_format($campo->boletos, 0, ',', '.')."' class='info'><span>$icono</span></td> \n";
