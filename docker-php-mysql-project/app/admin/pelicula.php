...
if($idp > 0):
	$qry = "SELECT id_pelicula, nombre_espanol, nombre_corto, nombre_ingles, id_distribuidor, duracion, 
			...
			id_formato_proyeccion, id_procedencia, activo,listar,listar_video,video FROM tbl_pelicula ";
	...
	if($campo = mysql_fetch_object($result)):
		...
		$listar = $campo->listar;
		$video = $campo->video;
		$listar_video = $campo->listar_video;
	...
	$listar_video = isset($_POST['listar_video']) ? $_POST['listar_video'] : '';
...
if($accion == AGREGAR && $status == 1):
	...
	$qry = "INSERT INTO tbl_pelicula (nombre_espanol, nombre_corto, nombre_ingles, id_distribuidor, duracion, 
			id_censura, pagina_web, id_genero, sinopsis, porcentaje_distribuidor, id_tipo, registro_obra, 
			id_formato_proyeccion, id_procedencia, activo,listar_video) VALUES (";
	$qry .= "'$nombre_espanol', '$nombre_corto', '$nombre_ingles','$distribuidor', '$duracion', '$censura', 
			'$pagina_web', '$genero', '$sinopsis', $porcentaje, $tipo, '$registro_obra', $id_formato_proyeccion, $id_procedencia, $activo,$listar_video)";
	...
	<td>
		<table width="100%" cellpadding="1" cellspacing="1" bgcolor="#FFFFFF">
			...
			<tr bgcolor="#DDDDDD">
				...
				<td width="81%"><input name="nombre_corto" type="text" class="css-campo-metadata" size="30" maxlength="30" value="<?php echo isset($nombre_corto) ? $nombre_corto : '';?>">
			...
			<tr bgcolor="#DDDDDD">
				...
				<tr bgcolor="#DDDDDD">
				<td width="19%" class="css-label-metadata">Listar Video:</td>
				<td width="81%">
                <?php
				$selected1=($listar_video==1) ? "selected='selected'" : "";
				$selected2=($listar_video==2) ? "selected='selected'" : "";
				echo "<select  name='listar_video' class='css-campo-metadata' >;";
				echo"<option value='1' $selected1>NO</option>";
				echo"<option value='2' $selected2>SI</option>";
				echo "</select>";
				?> 
                </td>
			</tr>
