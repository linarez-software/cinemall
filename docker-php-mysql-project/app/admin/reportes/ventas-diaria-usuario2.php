...
$qry = "SELECT t.usuario_registro, u.nombre_completo, SUM( t.cantidad_transaccion *dosporuno) AS boletos, SUM( t.total_transaccion ) AS monto, SUM(t.comision*t.cantidad_transaccion) as comision, tp.fecha_programacion, c.nombre as cine ,sum(t.efectivo) efectivo,sum(t.tarjeta) tarjetas,sum(t.dolar-(cambio/tasa)) dolar,sum(t.tasa*(t.dolar-(cambio/tasa))) dolarbs,sum(t.pm) pm ";
	...
	if(mysql_num_rows($result) > 0):
		...
		while($campo=mysql_fetch_object($result)):
			...
			if($cine_actual != $campo->cine):
				...
				$table .= "<td align='center' height='21' class='encabezado-reporte' >DIVISAS</td> \n";
				$table .= "<td align='center' height='21' class='encabezado-reporte' >DIVISAS BS</td> \n";
			...
			$table .= "<tr> \n";
			$table .= "<td width='10%' align='center' class='info' height='32'>".date("d/m/Y", strtotime($campo->fecha_programacion))."</td> \n";
			$table .= "<td width='10%' align='center' class='info'>".htmlentities($campo->nombre_completo)."(".$campo->usuario_registro.")" ."</td> \n";
			$table .= "<td width='10%' align='center' class='info'>".number_format($campo->boletos, 0, ',', '.')."</td> \n";
			$monto = $campo->monto+$campo->comision;
			$table .= "<td width='12%' align='right' class='info'>".number_format($monto, 2, ',', '.')."</td> \n";
			$table .= "<td width='12%' align='center' class='info'>".number_format($campo->efectivo, 2, ',', '.')."</td> \n";
			$table .= "<td width='12%' align='center' class='info'>".number_format($campo->dolar, 2, ',', '.')."</td> \n";
			$table .= "<td width='12%' align='center' class='info'>".number_format($campo->dolarbs, 2, ',', '.')."</td> \n";
			$table .= "<td width='12%' align='center' class='info'>".number_format($campo->tarjetas, 2, ',', '.')."</td> \n";
			$table .= "<td width='12%' align='center' class='info'>".number_format($campo->pm, 2, ',', '.')."</td> \n";
			$table .= "<td ></td> \n";
			$table .= "</tr> \n";
			$table .= "<tr> \n";
			$table .= "<td width='100%' colspan='10' align='center'  class='encabezado-reporte'  height='32'><b>Detalle ".htmlentities($campo->nombre_completo)."(".$campo->usuario_registro.")" ."</b></td> \n";
			$table .= "</tr> \n";
			$qry = "SELECT id_operacion,t.usuario_registro, u.nombre_completo,SUM( t.tarjeta+t.efectivo+t.pm+(t.dolar*t.tasa ) ) AS monto, SUM(t.comision*t.cantidad_transaccion) as comision, tp.fecha_programacion, c.nombre as cine ,sum(t.efectivo) efectivo,sum(t.tarjeta ) tarjetas ,sum(t.tasa*(t.dolar-(cambio/tasa))) dolar  ,sum(t.tasa*(t.dolar-(cambio/tasa)) ) dolarbs ,	t.hora_operacion,sum(t.pm ) pm ";
				$qry .= "FROM tbl_operacion t   ";
				$qry .= "INNER JOIN tbl_programacion tp ON t.id_programacion = tp.id_programacion ";
				$qry .= "INNER JOIN tbl_usuario u ON u.username = t.usuario_registro ";
				$qry .= "INNER JOIN tbl_cine c ON c.id_cine = t.id_cine ";
				$qry .= "INNER JOIN tbl_programacion pr ON t.id_programacion = pr.id_programacion ";
				$qry .= "WHERE DATE(t.fecha_operacion) = '$fecha_actual' and transmitido=1 ";
				$qry .= "AND t.status IN (1,2) ";
				if($sala > 0):
					$qry .= "AND tp.id_sala = $sala ";
				endif;
				...
				$qry .= "AND t.usuario_registro = $campo->usuario_registro ";
				if($pelicula > 0):
					$qry .= "AND pr.id_pelicula = $pelicula ";
				endif;
				$qry .= "GROUP BY id_operacion,t.usuario_registro, u.nombre_completo, tp.fecha_programacion,	t.hora_operacion ";
				$qry .= "ORDER BY id_operacion,u.nombre_completo,  tp.fecha_programacion  ";
				
			$table .= "<tr> \n";
			$table .= "<td width='100%' colspan='10' align='center' class='info' height='32'>
			<table width='100%' align='center' cellpadding=0 cellspacing=0 border=0>";
			$table .= "<tr> \n";
			$table .= "<td  align='right' class='encabezado-reporte'><b>Monto</b></td>\n";
			$table .= "<td  align='right' class='encabezado-reporte'><b>Efectivo</b></td>\n";
			$table .= "<td  align='right' class='encabezado-reporte'><b>Divisas</b></td>\n";
			$table .= "<td  align='right' class='encabezado-reporte'><b>Divisas bs</b></td>\n";
			$table .= "<td  align='right' class='encabezado-reporte'><b>Tarjetas</b></td>\n";
			$table .= "<td  align='right' class='encabezado-reporte'><b>Pago Movil</b></td>\n";
			$table .= "<td  align='right' class='encabezado-reporte'><b>Hora</b></td>\n";
			$table .= "</tr> \n";
			$result2 = mysql_query($qry) or die("Error en Consulta: " . mysql_error());
			while($campo2=mysql_fetch_object($result2)):
				$table .= "<tr> \n";
				$table .= "<td align='right'  class='info'>".number_format($campo2->monto, 2, ',', '.')."</td>\n";
				$table .= "<td align='right'  class='info'>".number_format($campo2->efectivo, 2, ',', '.')."</td>\n";
				$table .= "<td align='right'  class='info'>".number_format($campo2->dolar, 2, ',', '.')."</td>\n";
				$table .= "<td align='right'  class='info'>".number_format($campo2->dolarbs, 2, ',', '.')."</td>\n";
				$table .= "<td align='right'  class='info'>".number_format($campo2->tarjetas, 2, ',', '.')."</td>\n";
				$table .= "<td align='right'  class='info'>".number_format($campo2->pm, 2, ',', '.')."</td>\n";
				$table .= "<td align='right'  class='info'>".($campo2->hora_operacion)."</td>\n";
				$table .= "</tr> \n";
		...
		$table .= "<td align='center' class='encabezado-reporte'>".number_format($total_cine_boleto, 0, ',', '.')."</td> \n";
		$table .= "<td align='right' class='encabezado-reporte'>".number_format($total_cine_monto, 2, ',', '.')."</td> \n";
		$table .= "<td align='right' class='encabezado-reporte'>".number_format($total_e, 2, ',', '.')."</td> \n";
		$table .= "<td align='right' class='encabezado-reporte'>".number_format($total_d, 2, ',', '.')."</td> \n";
		$table .= "<td align='right' class='encabezado-reporte'>".number_format($total_dbs, 2, ',', '.')."</td> \n";
		$table .= "<td align='right' class='encabezado-reporte'>".number_format($total_t, 2, ',', '.')."</td> \n";
		$table .= "<td align='right' class='encabezado-reporte'>".number_format($total_p, 2, ',', '.')."</td> \n";
		$table .= "<td ></td> \n";
		$table .= "</tr> \n";
