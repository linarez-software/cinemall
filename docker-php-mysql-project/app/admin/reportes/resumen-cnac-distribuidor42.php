...
<table width="70%" align="left" cellpadding="0" cellspacing="0" border="0">
	...
	<?php
		...
		$table .= "</tr> \n";
				...
				$table .= "<tr> \n";
				$table .= "<td align='center' height='21' class='encabezado-reporte' >Tipo</td> \n";
				$table .= "<td align='center' height='21' class='encabezado-reporte' >Cantidad</td> \n";
				$table .= "<td align='center' height='21' class='encabezado-reporte' >Base Imponible del Boleto Neto</td> \n";
				$table .= "<td align='center' height='21' class='encabezado-reporte' >Total</td> \n";
				$table .= "</tr> \n";

			
			$qry = "SELECT	pr.nombre, 
							o.costo as precio,
							pr.foncine as imp,
			...
			$qry .= "WHERE  o.id_programacion in ($qry1) AND pr.costo > 1 ";
			$qry .= "GROUP BY nombre, pr.costo,pr.foncine,municipal,iva ";
			...
			if(mysql_num_rows($r_operacion) > 0):
				while($campo2=mysql_fetch_object($r_operacion)):
					$table .= "<tr> \n";
					$table .= "<td align='center' height='21' class='reporte'>".$campo2->nombre."</td> \n";
					$table .= "<td align='right' height='21' class='reporte'>".number_format($campo2->espectador, 2, ',', '.')."</td> \n";
					$iva=$campo2->iva;
					$isep=$campo2->municipal;
					$recaudacion=($campo2->precio *($iva+100))/(100+$isep+$iva);
					$recaudacion=($recaudacion *100)/($iva+100);
					$table .= "<td align='center' height='21' class='reporte'>".number_format($recaudacion, 2, ',', '.')."</td> \n";
					...
					$table .= "<td align='right' height='21' class='reporte'>".number_format($recaudacion*$campo2->espectador, 2, ',', '.')."</td> \n";
					$table .= "</tr> \n";
				endwhile;
			else:
