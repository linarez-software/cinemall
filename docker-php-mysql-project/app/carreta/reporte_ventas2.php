...
<form method="post" name="form1" action="" >
			...
			echo"<td  valign='middle' bgcolor='#F1F1F1' class='titulo' align='center' colspan='8'><b>Ventas</b></td>";
			echo"</tr>";
			echo"<tr  valign='middle' height=30>";
			echo"<td  valign='middle' bgcolor='#F1F1F1' class='titulo' align='center' colspan='8'><b>Listado de Ventas<br>Desde:&nbsp;".$fec[2]."/".$fec[1]."/".$fec[0]."&nbsp;&nbsp;&nbsp;Hasta:&nbsp;".$fec2[2]."/".$fec2[1]."/".$fec2[0]." </b></td>";
			echo"</tr>";
			echo"<tr  valign='middle' height=30>";
			echo"<td  valign='middle' bgcolor='#F1F1F1' class='titulo' align='center' colspan='8'><b>Almacen: $dep</b></td>";
			echo"</tr>";
			...
			while ($fila=mysql_fetch_array($result)){
					  ...
					  $sql="select sum(pv_tbl_pagos.MONTO_PAGO*tasa) FROM pv_tbl_pagos		  where (pv_tbl_pagos.CODIGO_INSTRUMENTO_PAGO=1 or pv_tbl_pagos.CODIGO_INSTRUMENTO_PAGO=6) and id_MOVIMIENTO=$fila[5]";
					  $result2 = mysql_query($sql);
					  $x=0;
					  while ($fila2=mysql_fetch_array($result2)){
					  	$x=$fila2[0];
					  }
					  echo"	<td valign='middle' align='center'>".str_replace("*",".",str_replace(".",",",str_replace(",","*",number_format($x,2))))."</td>";	
					  $te+=$x;	
					  $sql="select sum(pv_tbl_pagos.MONTO_PAGO) FROM pv_tbl_pagos		  where pv_tbl_pagos.CODIGO_INSTRUMENTO_PAGO=5 and id_MOVIMIENTO=$fila[5] and TIPO_tarjeta<>'PA'";
					  $result2 = mysql_query($sql);
					  $x=0;
					  while ($fila2=mysql_fetch_array($result2)){
					  	$x=$fila2[0];
