...
function valida()
{
	...
	if ((document.form1.cantidad.value<=0))
	 ...
	 document.form1.action="add_salidas_aux.php?insert=true";
	 document.form1.submit();
}
function anula(id)
{
	if (confirm("¿Esta seguro de eliminar este producto?")) {
	   ...
	   document.form1.action="add_salidas_aux.php?elimina=true&id="+ id ;
	   document.form1.submit();
	}
}
...
<?php	
			...
			{
					if($_POST[unidad]==1) {
						$sql="insert into aux_salida_detalle value(NULL,$_POST[producto],$_POST[cantidad],0,0,$_SESSION[login_usu]);";	
					}
					else
						{
						if($_POST[unidad]==2) {
								$sql="insert into aux_salida_detalle value(NULL,$_POST[producto],($_POST[cantidad]*(select CANTIDAD_UNIDAD_AGRUPADA from pv_tbc_productos where CODIGO_PRODUCTO=$_POST[producto] )),0,0,$_SESSION[login_usu]);";								}
							else
								{
								$sql="insert into aux_salida_detalle value(NULL,$_POST[producto],($_POST[cantidad]*(select display from pv_tbc_productos where CODIGO_PRODUCTO=$_POST[producto] )),0,0,$_SESSION[login_usu]);";	
								}
						
						}
					$result2 = mysql_query($sql);
					echo"<script>document.form1.action='add_salidas_aux.php';</script>";	
					echo"<script>document.form1.submit();</script>";
			}
			if ($_GET["elimina"]==true && isset($_GET["elimina"]))
			{
					$sql="delete from aux_salida_detalle where id=$_GET[id]";	
					$result2 = mysql_query($sql);
					echo"<script>document.form1.action='add_salidas_aux.php';</script>";	
					echo"<script>document.form1.submit();</script>";
			}
      ...
      <tr>
        ...
        <?php
				echo"<table width='100%' border='1' cellspacing='0' cellpadding='0'>";
					...
					$sql="SELECT
						pv_tbc_productos.DESCRIPCION_PRODUCTO,
						aux_salida_detalle.id_producto,
						aux_salida_detalle.id,
						aux_salida_detalle.cantidad,
						pv_tbc_productos.CANTIDAD_UNIDAD_AGRUPADA,
						pv_tbc_unidades.DESCRIPCION_UNIDAD_AGRUPADA,
						pv_tbc_productos.codigo_barra
						FROM
						aux_salida_detalle
						Inner Join pv_tbc_productos ON aux_salida_detalle.id_producto = pv_tbc_productos.CODIGO_PRODUCTO
