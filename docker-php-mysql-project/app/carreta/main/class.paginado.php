...
class paginado
{
	...
	function query($query)
	{
		...
		$query_count =  "SELECT COUNT(*) FROM (".$query.") a";
		...
		//$query .= " LIMIT ".(($this->pagina()-1)*$this->porPagina()).",".$this->porPagina();
	...
	function siguiente()
	{
		switch (func_num_args())
		{
			...
			default:
				...
				{
					return "<a href=\"?pagina=".($this->pagina()+1).$this->propagar()."&txt=".$this->text."\">".$this->_siguiente."</a>";
				}// Fin If
			break;
		}
	} // function	
	...
	function anterior()
	{
		switch (func_num_args())
		{
			...
			default:
				if($this->pagina() != 1)
				{
					return "<a href=\"?pagina=".($this->pagina()-1).$this->propagar()."&txt=".$this->text."\">".$this->_anterior."</a>";
				}// Fin If
			break;
		}
	} // function
	...
	function nroPaginas()
	{
		...
		{
			$temp[$i] = "<a href=\"?pagina=$i".$this->propagar()."&txt=".$this->text."\">$i</a>";
		} // for
