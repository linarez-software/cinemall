...
function anula(id)
{
	if (confirm("¿Esta seguro de Anular esta entrada?")) {
	   ...
	   document.form1.action="entradas.php?elimina=true&id="+ id ;
	   document.form1.submit();
	}
}
function valida()
{
	   window.open ("add_entradas.php", "Help","scrollBars=yes,resizable=no,toolbar=no,menubar=no,top =10,left=10,location=no,directories=no,width=900,height=800");  

}

function vista(id)
{
	   document.form1.action="reporte_entrada.php?id="+ id ;
	   document.form1.submit();

}
function vista2(id)
{
		window.open ("reporte_entrada2.php?id="+ id, "Help","scrollBars=yes,resizable=no,toolbar=no,menubar=no,top =10,left=10,location=no,directories=no,width=750,height=800");  
...
<tr>
	...
	<form method="post" name="form1" action="Consulta_facturas.php" >
            ...
            <?php
			  ...
			  if ($_GET["elimina"]==true && isset($_GET["elimina"]))
				{
					...
					WHERE
					TIPO_MOVIMIENTO='ENT' and (c.EXISTENCIA_PRODUCTO-b.CANTIDAD_UNITARIA<0)and a.ID_MOVIMIENTO=".$_GET["id"];
					...
					if($a==0){
						...
						echo"<script>document.form1.action='entradas.php';</script>";	
						echo"<script>document.form1.submit();</script>";	
					}
					else
						{
						echo"<script>alert('No hay Existencia para anular la entrada');</script>";
						echo"<script>document.form1.action='entradas.php';</script>";	
						echo"<script>document.form1.submit();</script>";	
						}
				}
			  ...
			  echo"<tr>";
			  echo"	<td bgcolor='#F1F1F1' class='titulo' colspan='8' align='center'>Listado de Entradas de Inventario</td>";
			  echo"</tr>";
			  echo"<tr>";
