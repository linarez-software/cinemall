...
function cancelar()
{
   document.form1.action="add_salidas.php?cancela=true";
...
function valida()
{
	if ((document.form1.fecha.value==""))
	 ...
	 {
	 	alert("Debe indicar el Destino de la salida");
		return false;
	 }
	...
	document.form1.action="add_salidas.php?insert=true";
	 document.form1.submit();
}
			...
			if ($_GET["insert"]==true && isset($_GET["insert"]))
			{
				...
				else
				{
					...
					$sql="SELECT
						...
						aux_salida_detalle.cantidad,c.CODIGO_PRODUCTO
						FROM
						pv_tbc_inventarios AS c
						Inner Join aux_salida_detalle ON aux_salida_detalle.id_producto = c.CODIGO_PRODUCTO

					WHERE
					(c.EXISTENCIA_PRODUCTO-cantidad<0)and aux_salida_detalle.id_usuario=$_SESSION[login_usu] and CODIGO_ALMACEN=".$_POST["deposito"];
					$result = mysql_query($sql);
					$a=0;
					...
					if($a==0){
						...
						$sql=$sql."VALUES (NULL,'SAL','$nro','".$_POST["fecha"]."',0,'".$_POST["deposito"]."',0, '".$_POST["origen"]."',1,'".$login."','".$_POST["obs"]."', 'EMITIDA',0,CURTIME())";
						$result = mysql_query($sql);
						$id = mysql_insert_id();
						
						$sql="insert into pv_tbl_detalle_movimientos SELECT $id,pv_tbc_productos.CODIGO_BARRA, pv_tbc_productos.CODIGO_PRODUCTO, aux_salida_detalle.cantidad, pv_tbc_productos.DESCRIPCION_PRODUCTO, pv_tbc_productos.PRECIO, pv_tbc_impuestos.PORCENTAJE_IMPUESTO, pv_tbc_productos.CANTIDAD_UNIDAD_AGRUPADA, pv_tbc_productos.CODIGO_UNIDAD_AGRUPADA,0,0,NULL FROM aux_salida_detalle Inner Join pv_tbc_productos ON aux_salida_detalle.id_producto = pv_tbc_productos.CODIGO_PRODUCTO Inner Join pv_tbc_impuestos ON pv_tbc_productos.CODIGO_IMPUESTO = pv_tbc_impuestos.CODIGO_IMPUESTO  where id_usuario=$_SESSION[login_usu] ";
						$result = mysql_query($sql);
						...
						$sql="delete from aux_salida_detalle where id_usuario=$_SESSION[login_usu] ";
						$result = mysql_query($sql);
						
						echo"<script>";
						echo"alert('Salida Ingresada Satisfactoriamente');";
						echo"window.opener.location.href='salidas.php';";	
						echo"window.close();";
						echo"</script>";
						exit();
					}
						...
						{
						echo"<script>document.form1.action='add_salidas.php';</script>";	
...
?>
	...
	<table width="720" border="0" cellspacing="0" cellpadding="0">
      <tr>
        ...
        <td colspan="4">
        <iframe width="95%" height="450" src="add_salidas_aux.php" name="reporte" frameborder="0" scrolling="auto"></iframe>
