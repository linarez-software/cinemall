...
if($idf > 0):
	$qry_hora = "SELECT pr.id_programacion, pr.hora_inicio, c.nomenclatura, pe.id_censura, pe.id_tipo 
...
//echo $dia_semana;
	$qry_precios = "SELECT id_precio, nombre, costo, hora_limite, censura ";
...
function seleccionarButaca(sala){

	var alto = document.getElementById('fila').value;
	var ancho = document.getElementById('columna').value;
	
	 cpanelLink = document.getElementById("mapa"); 
	 Shadowbox.setup(cpanelLink, { 
	 	height: 	(alto+3)*63,
	 	width: 		(ancho+5)*51,
	 	player:     'iframe' 
	 	});
	/*
	 Shadowbox.open({
        player:     "html",
        title:      "Seleccionar Butaca",
        content:	'sala-mapa.php?ids='+sala,
        height:     (alto+3)*63,
        width:      (ancho+5)*51
    });*/
   
   //Shadowbox.setup('sala-mapa.php?ids='+sala, {});

}

function VentanaBoletos(id, costo){
	var strFeatures = "border=0; dialogWidth=290px; dialogHeight=360px; center=yes; help=no;"
	...
	if(boletos > 0){
		//if(numerada>0){
		//	var sala = document.all.id_sala.value;
		//	seleccionarButaca(sala);
		//} else {
		//	guardarVenta(id, costo, boletos);	
		//}
		guardarVenta(id, costo, boletos);
	}
}
...
if($consulta == 1):
	...
	while($campo_precios = mysql_fetch_object($precios_result)):
		...
		if($hora_limite >= time() or $campo_precios->hora_limite == "00:00:00"):
			$table .= "<tr>";
			$table .= "<a href='#' onClick='javascript:VentanaBoletos($campo_precios->id_precio, $campo_precios->costo)'>
			<td height='41' align='center' class='precios-precios' bgcolor='#999999'" .rowSelect($bgcolor,'out')." ".rowSelect($bgcolorselect,'over').">
			<a href='#' onClick='javascript:VentanaBoletos($campo_precios->id_precio, $campo_precios->costo)' class='precios-precios'>$campo_precios->nombre</a></a></td>";
			$table .= "</tr>";
		endif;
	endwhile;
	echo $table;
