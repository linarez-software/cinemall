...
$qry_led .= "t.cantidad_transaccion, t.id_precio, t.costo, t.comision, t.total_transaccion, t.usuario_registro, ";
	...
	if($consulta == 1):
		...
		while($campo = mysql_fetch_object($led_result)):
			...
			if(($campo->numerada==1)):
				...
				$alto = !empty($fila) ? (($fila + 3) * 60) : 0;
				$ancho = !empty($columna) ? (($columna + 5) * 50)  : 0;
			...
			$monto = ($campo->cantidad_transaccion * ($campo->costo + $campo->comision)); 
			$table .= "<td height='23' class='led-texto' valign='middle'><b>".$campo->cantidad_transaccion."</b>   X    ".$campo->comentario.", ".$campo->nombre_corto." - ".date("h:i a", strtotime($campo->hora_inicio)).", Bs. = <b>". number_format($monto,2,',','.')."</b>&nbsp;&nbsp;";
		...
		$table .= "<tr><td colspan='2' height='50' valign='middle' class='led-total'>TOTAL VENTA (Bs.)= <b>".number_format($total,2,',','.')."</b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;-&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;TOTAL VENTA ($)= <b>".number_format($total/$TASA,2,',','.')."</b></td></tr> \n";
		echo $table;
	endif;
	?>
	</div>
