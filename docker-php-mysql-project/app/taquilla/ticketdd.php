...
while($campo_venta = mysql_fetch_object($temp_venta)):
	...
	if($incompletas==0):
		...
		$qry_insert .= "$campo_venta->costo, $campo_venta->comision, $campo_venta->total_transaccion, $campo_venta->usuario_registro, 1, $campo_venta->porcentaje_distribuidor, '$hora_operacion','$municipal',12,'$foncine') ";
		//echo $qry_insert;
		$result_insert = mysql_query($qry_insert) or die(mysql_error());
		
		$sql="insert into tbl_encabezado_detalle values(NULL,$id_movimiento,'ENTRADAS $cant_imp','$precio_imp','12','$cant_imp','$municipal')";
		$rest = mysql_query($sql); 
...
{
	////IMPRESORA FISCAL
	/*$sql="insert into tbl_encabezado_detalle values(NULL,$id_movimiento,'ENTRADAS $cant_imp','$precio_imp','12','1','2')";
	$rest = mysql_query($sql); */
