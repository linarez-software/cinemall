...
$municipal = 2;
$foncine =5;
$qry = "SELECT nombre_empresa, direccion, rif, nit,municipal,fomprocine,isv ";
...
$montotarjeta=$_GET["monto"]=='' ? 0 :$_GET["monto"];
$montoefectivo=$_GET["efectivo"]=='' ? 0 :$_GET["efectivo"];
$trasmitido=1;
$cedula=$_GET["cedula"]=='' ? 0 :$_GET["cedula"];
$nombre=$_GET["nombre"]=='' ? 'CLIENTE' : strtoupper($_GET["nombre"]);
$pm=$_GET["pm"]=='' ? 0 :$_GET["pm"];
$ref=$_GET["ref"];
$banco=$_GET["banco"];
...
$precio_imp =0;
$total_imp_f =0;
//$pago=$montotarjeta+$montoefectivo+($dolar*$tasa);
$idope="";
$dolar=$_GET["dolar"];
$tasa=$_GET["tasa"];

$pago=$montotarjeta+$montoefectivo+($dolar*$tasa)+ $pm;
$qry = " SELECT ";
$qry .= " sum(t.cantidad_transaccion*p.dosporuno), sum(t.total_transaccion), t.usuario_registro";
$qry .= " FROM temp_operacion t INNER JOIN tbl_precio p ON t.id_precio = p.id_precio ";
$qry .= " WHERE id_session = '" . $_SESSION['id'] ."' group by  t.usuario_registro ";

$temp = mysql_query($qry);
while($campo = mysql_fetch_array($temp)){
		$sql="insert into tbl_operacion_resumen values(NULL,'".date("Y-m-d")."','".date("H:i:s")."','".$campo[1]."','".$campo[0]."','".$montotarjeta."','".$montoefectivo."','".$dolar."','".$tasa."','".$campo[2]."','".$pm."')";
		$s=mysql_query($sql);
}
...
while($campo_venta = mysql_fetch_object($temp_venta)):
	...
	if($incompletas==0):
		...
		$qry_insert .= "costo, comision, total_transaccion, usuario_registro, status, porcentaje_distribuidor, hora_operacion,municipal,iva,foncine,transmitido,tarjeta,efectivo,dolar,tasa,pm,ref,banco) VALUES ( ";
		...
		$qry_insert .= "$campo_venta->costo, $campo_venta->comision, $campo_venta->total_transaccion, $campo_venta->usuario_registro, 1, $campo_venta->porcentaje_distribuidor, '$hora_operacion','$municipal','$isv','$foncine',$trasmitido,'".($montotarjeta)."','$montoefectivo','".($dolar)."','$tasa','$pm','$ref','$banco') ";
		//echo $qry_insert;
		$montotarjeta=0;
		$trasmitido=0;
		$montoefectivo=0;
		...
		if ($idope=="")	$idope=$idope.$id_operacion.")";
		...
		$qry_id2 = "SELECT porcentaje,pe.id_procedencia FROM tbl_pelicula pe INNER JOIN tbl_programacion  pr  on  pe.id_pelicula = pr.id_pelicula inner join tbl_operacion op ON op.id_programacion = pr.id_programacion inner join tbl_distribuidor d ON d.id_distribuidor = pe.id_distribuidor WHERE id_operacion=$id_operacion ";
		...
		$qry_insert = "update  tbl_operacion set foncine='$pd'  where  id_operacion=$id_operacion ";
		$result_id2 = mysql_query($qry_insert);

		$qry_id2 = "SELECT nombre_espanol FROM tbl_pelicula pe INNER JOIN tbl_programacion  pr  on  pe.id_pelicula = pr.id_pelicula inner join tbl_operacion op ON op.id_programacion = pr.id_programacion WHERE id_operacion=$id_operacion ";
		$result_id2 = mysql_query($qry_id2);
		$nombre_pelicula = "";
		if($campo_id2 = mysql_fetch_object($result_id2)):
			$nombre_pelicula = $campo_id->nombre_espanol;
		endif;
		$nombre_pelicula=strtoupper($nombre_pelicula);
		$pos = strpos($nombre_pelicula, "COMBO");
		if ($pos === false) {
			$total_imp_f +=($campo_venta->cantidad_transaccion*$campo_venta->costo);
			$sql="insert into tbl_encabezado_detalle values(NULL,$id_movimiento,'ENTRADAS $cant_imp','$precio_imp','$isv','$cant_imp','$campo_venta->foncine')";
			$rest = mysql_query($sql); 
		}
		...
		$qry_datos .= "p.nombre_espanol as pelicula,p.nombre_corto as corto, s.nombre as sala ,p.id_tipo ,(fecha_programacion > CURDATE()) preventa  ";
		...
		if($campo_datos = mysql_fetch_object($result_datos)):
			$preventa=($campo_datos->preventa==1) ? "PREVENTA" :"";
			...
			$dia = $dias[(date('N', strtotime($campo_datos->fecha))) - 1];
			?>
			...
			<span class="impuesto">.</span><br>
				...
				?>
				<tr>
			    	<td height="25 " width="250" valign="middle" align="center" style="border-top: 1px dashed #000;">
				...
				?>
			    <tr>
			    	<td width="250" height="10" valign="middle" align="center">
			        	<span class="pelicula"><b><?php echo $preventa;?></b></span><br>
			        </td>
			    </tr>
			    <tr>
...
endwhile;
$sql="update  tbl_operacion	 set cambio='$cambio' where id_operacion in($idope";

$rest = mysql_query($sql);
/*$qry_led = "SELECT
			pv_tbc_productos.CODIGO_PRODUCTO, 
			temp_operacion_combo.cantidad, 
			temp_operacion_combo.PRECIO, 
			temp_operacion_combo.dolar,id_operacion_combo,DESCRIPCION_PRODUCTO
		FROM
			temp_operacion_combo
			INNER JOIN
			pv_tbc_productos
			ON 
				temp_operacion_combo.CODIGO_PRODUCTO = pv_tbc_productos.CODIGO_PRODUCTO";
$qry_led .= " WHERE id_session = '" . $_SESSION['id']  ."'";
$led_result = mysql_query($qry_led);
while($campo = mysql_fetch_object($led_result)):
	$sql="insert into tbl_operacion_combo values(NULL,'".date("Y-m-d")."','$campo->CODIGO_PRODUCTO','$campo->cantidad','$campo->PRECIO','$campo->dolar')";
	$led_result2 = mysql_query($sql);
	$total_imp_f=$total_imp_f+($campo->cantidad*$campo->PRECIO);
	$sql="insert into tbl_encabezado_detalle values(NULL,$id_movimiento,'$campo->DESCRIPCION_PRODUCTO','$campo->PRECIO','$isv','$campo->cantidad','0')";
	$rest = mysql_query($sql); 
endwhile;*/
$cambio=$pago-$total_imp_f;
$cambio=($cambio<0) ?0 : $cambio;

/*$qry_delete = "DELETE FROM temp_operacion_combo WHERE id_session = '" . $_SESSION['id']  ."'";
$result_delete = mysql_query($qry_delete) or die(mysql_error());*/
