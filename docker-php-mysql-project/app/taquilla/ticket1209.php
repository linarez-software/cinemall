...
//Datos Empresa
$municipal = 10;
$qry = "SELECT nombre_empresa, direccion, rif, nit,municipal ";
...
$qry .= "p.nombre, p.comentario, butacas, numerada, p.contenido, p.marcar, p.marca ";
...
while($campo_venta = mysql_fetch_object($temp_venta)):
	...
	if($incompletas==0):
		...
		$precio_imp +=($campo_venta->cantidad_transaccion*$campo_venta->costo);
		...
		$qry_insert .= "costo, comision, total_transaccion, usuario_registro, status, porcentaje_distribuidor, hora_operacion,municipal,iva) VALUES ( ";
		...
		$qry_insert .= "$campo_venta->costo, $campo_venta->comision, $campo_venta->total_transaccion, $campo_venta->usuario_registro, 1, $campo_venta->porcentaje_distribuidor, '$hora_operacion','$municipal',12) ";
		//echo $qry_insert;
...
{
	////IMPRESORA FISCAL
	$sql="insert into tbl_encabezado_detalle values(NULL,$id_movimiento,'ENTRADAS $cant_imp','$precio_imp','12','1')";
	$rest = mysql_query($sql); 
	$sql="update  tbl_encabezado set monto='$precio_imp' where id=$id_movimiento";
	$rest = mysql_query($sql);
...
else
{
	...
	$sql="insert into tbl_encabezado_detalle values(NULL,$id_movimiento,'ENTRADAS $cant_imp','$precio_imp','12','1')";
	$rest = mysql_query($sql); 
	$sql="update  tbl_encabezado set monto='$precio_imp',impresa=1 where id=$id_movimiento";
	$rest = mysql_query($sql);
