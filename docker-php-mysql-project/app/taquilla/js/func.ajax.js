...
function guardarVenta(precio, costo, comision, boletos)
{
	var session=document.getElementById("session").value;
	var username=document.getElementById("username").value;
	var funcion=document.getElementById("funcion").value;
	...
	//var costo=document.getElementById("costo").value;
	//var boletos=document.getElementById("boletos").value;
	var url = "session=" + session + "&username=" + username + "&funcion=" + funcion + "&precio=" + precio + "&costo=" + costo+ "&comision=" + comision + "&boletos=" + boletos;
	//alert(url);
	ajax=nuevoAjax();
	ajax.open("GET", "venta-temp.php?"+url, true);
	ajax.onreadystatechange=function() 
