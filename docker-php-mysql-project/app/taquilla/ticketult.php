...
while($campo_venta = mysql_fetch_object($temp_venta)):
	...
	if($incompletas==0):
		...
		$qry_insert .= "costo, comision, total_transaccion, usuario_registro, status, porcentaje_distribuidor, hora_operacion,municipal,iva,foncine,transmitido,tarjeta,efectivo) VALUES ( ";
		...
		$qry_insert .= "$campo_venta->costo, $campo_venta->comision, $campo_venta->total_transaccion, $campo_venta->usuario_registro, 1, $campo_venta->porcentaje_distribuidor, '$hora_operacion','$municipal','$isv','$foncine',$trasmitido,'".($montotarjeta)."','$montoefectivo') ";
		//echo $qry_insert;
		$montotarjeta=0;
		$trasmitido=0;
		$montoefectivo=0;
