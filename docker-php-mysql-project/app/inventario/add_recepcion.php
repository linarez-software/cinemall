...
function cancelar()
{
   document.form1.action="add_recepcion.php?cancela=true";
...
function valida()
{
	if ((document.form1.fecha.value==""))
	 ...
	 {
	 	alert("Debe indicar el origen de la recepcion");
		return false;
	 }
	 if ((document.form1.deposito.value==""))
	 {
	 	alert("Debe indicar el deposito de la recepcion");
		return false;
	 }
	  if ((document.form1.nrofactura.value==""))
	 {
	 	alert("Debe indicar el nro de factura de la recepcion");
		return false;
	 }
	document.form1.action="add_recepcion.php?insert=true";
	 document.form1.submit();
}
function proveedor()
{
parent.reporte.location.href="add_recepcion_aux.php?prov=" + document.form1.origen.value;
}
</script>
<html>
<?php	
			if ($_GET["insert"]==true && isset($_GET["insert"]))
			{
				...
				else
				{
					...
					$sql=$sql."VALUES (NULL,'REC','$nro','".$_POST["fecha"]."',".$_POST["origen"].",0,'".$_POST["deposito"]."', '".$_POST["nrofactura"]."',1,'".$login."','".$_POST["obs"]."', 'EMITIDA',0,CURTIME())";
					$result = mysql_query($sql);
					$id = mysql_insert_id();
					
					$sql="insert into pv_tbl_detalle_movimientos SELECT $id,pv_tbc_productos.CODIGO_BARRA, pv_tbc_productos.CODIGO_PRODUCTO, aux_recepcion_detalle.cantidad, pv_tbc_productos.DESCRIPCION_PRODUCTO, pv_tbc_productos.PRECIO, pv_tbc_impuestos.PORCENTAJE_IMPUESTO, pv_tbc_productos.CANTIDAD_UNIDAD_AGRUPADA, pv_tbc_productos.CODIGO_UNIDAD_AGRUPADA,aux_recepcion_detalle.costo,aux_recepcion_detalle.descuento,NULL FROM aux_recepcion_detalle Inner Join pv_tbc_productos ON aux_recepcion_detalle.id_producto = pv_tbc_productos.CODIGO_PRODUCTO Inner Join pv_tbc_impuestos ON pv_tbc_productos.CODIGO_IMPUESTO = pv_tbc_impuestos.CODIGO_IMPUESTO  where id_usuario=$_SESSION[login_usu] ";
					$result = mysql_query($sql);
					...
					$sql="delete from aux_recepcion_detalle where id_usuario=$_SESSION[login_usu] ";
					$result = mysql_query($sql);
					
					echo"<script>";
					echo"alert('Recepcion Ingresada Satisfactoriamente');";
					echo"window.opener.location.href='recepcion.php';";	
					echo"window.close();";
					echo"</script>";
					exit();
      ...
      <tr>
        <td align="center">
            ...
            <tr height="20">   
              ...
              <tr height="70">   
                <td class="TITULO" width="20%">&nbsp;&nbsp;Nro Factura:</td>
                <td width="30%" align="left" colspan="3">
				<input type="text" name="nrofactura" value="" maxlength="20" size="20" />
				</td>
        	</tr>
            </tr>
              <tr height="70">   
        ...
        <td colspan="4">
        <iframe width="100%" height="450" src="add_recepcion_aux.php?prov=<?php echo $prov;?>" name="reporte" frameborder="0" scrolling="auto"></iframe>
