...
function Validar(id)
{
 ...
 document.form1.action="edit_categoria.php?insert=0&id=" +id;
...
if ($_GET["insert"]==0)
{
	...
	$sql="update pv_tbc_categoria set ";
		$sql.="descripcion_categoria ='".strtoupper($_POST[producto])."' ";
		$sql.=" where codigo_categoria =$_GET[id] ";
    $result = mysql_query($sql);
	mysql_close ($enlace);
	echo"<script>window.opener.location.href='categoria.php';</script>";	
	echo"<script>window.close();</script>";
}
else
{
	$query="SELECT *
             FROM pv_tbc_categoria 
             WHERE codigo_categoria=$_GET[id]";
