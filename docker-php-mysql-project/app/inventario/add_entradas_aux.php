...
function valida()
{
	...
	if ((document.form1.cantidad.value<=0))
	 ...
	 document.form1.action="add_entradas_aux.php?insert=true";
	 document.form1.submit();
}
function anula(id)
{
	if (confirm("¿Esta seguro de eliminar este producto?")) {
	   ...
	   document.form1.action="add_entradas_aux.php?elimina=true&id="+ id ;
	   document.form1.submit();
	}
}
...
<?php	
			...
			{
					if($_POST[unidad]==1) {
						$sql="insert into aux_entradas_detalle value(NULL,$_POST[producto],$_POST[cantidad],0,0,$_SESSION[login_usu]);";	
					}
					else
						{
						if($_POST[unidad]==2) {
							$sql="insert into aux_entradas_detalle value(NULL,$_POST[producto],($_POST[cantidad]*(select CANTIDAD_UNIDAD_AGRUPADA from pv_tbc_productos where CODIGO_PRODUCTO=$_POST[producto] )),0,0,$_SESSION[login_usu]);";	
						}
						else
							{
							$sql="insert into aux_entradas_detalle value(NULL,$_POST[producto],($_POST[cantidad]*(select display from pv_tbc_productos where CODIGO_PRODUCTO=$_POST[producto] )),0,0,$_SESSION[login_usu]);";	
							}
						}
					$result2 = mysql_query($sql);
					echo"<script>document.form1.action='add_entradas_aux.php';</script>";	
					echo"<script>document.form1.submit();</script>";
			}
			if ($_GET["elimina"]==true && isset($_GET["elimina"]))
			{
					$sql="delete from aux_entradas_detalle where id=$_GET[id]";	
					$result2 = mysql_query($sql);
					echo"<script>document.form1.action='add_entradas_aux.php';</script>";	
					echo"<script>document.form1.submit();</script>";
			}
      ...
      <tr>
        ...
        <?php
				echo"<table width='100%' border='1' cellspacing='0' cellpadding='0'>";
					...
					echo"	<td width='20%' bgcolor='#F1F1F1' class='TITULO' align='center'>Unidad Agrupada</td>";
					...
					$sql="SELECT
						pv_tbc_productos.DESCRIPCION_PRODUCTO,
						aux_entradas_detalle.id_producto,
						aux_entradas_detalle.id,
						aux_entradas_detalle.cantidad,
						...
						Inner Join pv_tbc_productos ON aux_entradas_detalle.id_producto = pv_tbc_productos.CODIGO_PRODUCTO
					...
					$i=1;
					 while ($rs=mysql_fetch_array($result)){
						...
						echo"	<td class='TITULO' align='center'>$rs[5]</td>";
						echo"	<td align='center'>".number_format(($rs[3]/$rs[4]),2)."</td>";
