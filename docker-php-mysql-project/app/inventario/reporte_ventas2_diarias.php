			...
			while ($fila=mysql_fetch_array($result)){
					  if ($fila["CODIGO_CAJA"]==99)
					  {
						  ...
						  while ($fila1=mysql_fetch_array($result1)){	
								...
								if($fila1[1]==6)$te+=$fila1[3];

								if($fila1[1]==5 && $fila1[4]=='PA')$tc+=$fila1[3];
								if($fila1[1]==5 && $fila1[4]!='PA')$tt+=$fila1[3];
						 ...
						 $result1 = mysql_query($sql);
						  ...
						  echo"	<td valign='middle' align='center' class='contenido2' >".str_replace("*",".",str_replace(".",",",str_replace(",","*",number_format($igft+$fila[3],2))))."</td>";	
						  $t+=$fila[3]+$igft;	
						  $tte+=$te;	
						  $ttt+=$tt;	
						  $ttc+=$tc;
