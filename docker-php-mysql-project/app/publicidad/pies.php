...
$qry_precios .= "FROM tbl_precio ";
 $qry_precios .= "WHERE activo = 1  AND costo > 1 AND SUBSTRING(dia, $dia_semana+1, 1) = 1 AND (LENGTH(contenido) = 0 OR contenido IS NULL) and not nombre like '%VIP%'";
$precios_result = mysql_query($qry_precios);
$cantidad = mysql_num_rows($precios_result);
//$cantidad = floatval($cantidad) / 3;

?>
<table id="tabla-pies" width="99%" cellpadding="0" cellspacing="0"  border="0" align="center">
<tr>
...
while($campo_precios = mysql_fetch_object($precios_result)):
	...
	if($hora_limite >= time() or $campo_precios->hora_limite == "00:00:00"):
		...
		$table .= "<tr><td valign='middle' align='left' class='precio-detalle'><b>" . strtolower($campo_precios->nombre) .":&nbsp;</b>";
