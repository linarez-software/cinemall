...
$qry .= "WHERE id_sala in (select sala from tbl_mostrar_sala) and fecha_programacion = '$fecha' ";
...
<table id="tabla-pelicula" width="100%" height="60%" cellpadding="0" cellspacing="0" border="0">
<?php 
$table = "";
$par = 0;
for($p=1; $p<=$intervalo; $p++):
	if($campo = mysql_fetch_object($programacion)):
		...
		$table .= "<tr class='pelicula $tr_fondo' height='20'> \n"; 
		...
		$qry .= "AND TIME_TO_SEC(hora_inicio) + ".($tiempo_expiracion * 60). " >= TIME_TO_SEC(now()) and id_sala in (select sala from tbl_mostrar_sala) ";
		...
		for($i=1; $i<=5; $i++):
			$table .= "<td width='12%' class='borde-derecho borde-abajo $color' align='center' valign='middle'>&nbsp;</td> \n";
		endfor;
		$table .= "</tr> \n";
